...
class LOL_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # img_lq_print = tensor2img(input_img_pt, min_max=(-1, 1))
        # img_gt_print = tensor2img(gt_img_pt, min_max=(-1, 1))
        # img_print = np.concatenate([img_gt_print, img_lq_print], axis=1)
        # imwrite(img_print, os.path.join('/sdb/zdw/project/low_level/DEBUG/mit5k128_dataset_val', os.path.split(gt_path)[-1]))
...
if __name__ == '__main__':
    ...
    for x in dataloader:
        ...
        # from basicsr.utils import imwrite, tensor2img
        # img_lq_print = tensor2img(x['LR'][:, :3, :, :], min_max=(-1, 1))
        # img_hiseq_print = tensor2img(x['LR'][:, 3:6, :, :], min_max=(-1, 1))
        # img_gt_print = tensor2img(x['HR'][:, :, :, :], min_max=(-1, 1))
        # img_print = np.concatenate([img_gt_print, img_lq_print, img_hiseq_print], axis=1)
        # imwrite(img_print, os.path.join('/sdb/zdw/project/low_level_cp/DEBUG/exp24', os.path.split(x['lq_path'][0])[-1]))
        cnt += 1
        if cnt >= 30:
            break