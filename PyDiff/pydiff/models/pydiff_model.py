...
class PyDiffModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.csrnet = build_network(opt['network_csrnet'])
        self.csrnet = self.model_to_device(self.csrnet)
        opt['network_ddpm']['color_fn'] = self.csrnet
        ...
        load_path = self.opt['path'].get('pretrain_network_csr', None)
        if load_path is not None:
            param_key = self.opt['path'].get('param_key_csr', 'params')
            self.load_network(self.bare_model.color_fn, load_path, self.opt['path'].get('strict_load_csr', True), param_key)
        
        # load pretrained model
        load_path = self.opt['path'].get('pretrain_network_unet', None)
        if load_path is not None:
            param_key = self.opt['path'].get('param_key_unet', 'params')
            self.load_network(self.bare_model.denoise_fn, load_path, self.opt['path'].get('strict_load_unet', True), param_key)
